...
class RecipeSearchAPITest(TestCase):
    def setUp(self):
        ...
        recipe = Recipe.objects.create(title="Carbonara", description="Delicious pasta")
        recipe.cuisines.add(cuisine)
        

    def test_search_recipes(self):
        response = self.client.get(reverse('recipe-search') + "?q=carbonara&cuisine=Italian")
        self.assertEqual(response.status_code, 200)
        self.assertGreater(len(response.data), 0)  # Check that at least one recipe is returned
